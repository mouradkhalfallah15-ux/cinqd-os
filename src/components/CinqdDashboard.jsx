...
import { FiUsers, FiTrendingUp, FiPackage, FiDollarSign, FiCpu, FiActivity, FiArrowUpCircle, FiArrowDownCircle, FiLogOut } from 'react-icons/fi';
...
// TODO: Replace with real Firebase reads
const getMockData = () => ({
  production: {
    todayVolume: '1,250 L',
    change: '+5.2%',
...
const CinqdDashboard = () => {
  ...
  useEffect(() => {
    if (!user) return;
    const timer = setTimeout(() => setData(getMockData()), 500);
      ...
      <header className="p-6 border-b border-slate-800 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            disabled={loggingOut}
            title="تسجيل الخروج"
            className="flex items-center gap-2 text-slate-400 hover:text-red-400 disabled:opacity-50 transition-colors text-sm border border-slate-700 hover:border-red-500/50 rounded-lg px-3 py-2"
          >
            <FiLogOut />
            {loggingOut ? '...' : 'خروج'}
          </button>
      ...
      <main className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-2 bg-slate-900/50 backdrop-blur-sm border border-slate-800 rounded-2xl p-6 animate-in fade-in duration-500">
          <h3 className="text-lg font-bold text-white flex items-center gap-2 mb-4">
            <FiCpu className="text-cyan-400" /> AI Auditor Insights
          </h3>
        ...
        <div className="lg:col-span-2 bg-slate-900/50 backdrop-blur-sm border border-slate-800 rounded-2xl p-6 animate-in fade-in duration-500">
          <h3 className="text-lg font-bold text-white flex items-center gap-2 mb-4">
            <FiUsers className="text-cyan-400" /> Sales by Channel
          </h3>
        ...
        <div className="lg:col-span-4 bg-slate-900/50 backdrop-blur-sm border border-slate-800 rounded-2xl p-6 animate-in fade-in duration-500">
          <h3 className="text-lg font-bold text-white flex items-center gap-2 mb-4">
            <FiActivity className="text-cyan-400" /> Recent Production Batches
          </h3>
